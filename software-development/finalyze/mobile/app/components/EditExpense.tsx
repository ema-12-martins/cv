...
export default function EditExpense() {
  ...
  useEffect(() => {
    fetch('http://192.168.1.69:8080/api/expenses/1')
      ...
      .then((data) => {
        setExpenseCategory(data.labelId ? String(data.labelId) : 'Food');
        setExpenseAmount(String(data.value));
        setExpenseLabel(data.labelId ? `Category ${data.labelId}` : '');
        setExpensePriority('2');
        setExpenseDate(new Date(data.occurrenceDate));
      })
      .catch((error) => {
  ...
  const savePressed = () => {
    ...
    fetch('http://192.168.1.69:8080/api/expenses/1', {
      ...
      .then((response) => response.json())
      .then((data) => {
        alert('Expense updated successfully! ' + JSON.stringify(data));
      })
      .catch((error) => {
