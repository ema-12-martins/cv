	...
	*/
	...
	getFog(element){
		...
		let newObj = {
			elem: "fog",
			descriptor: "fog",
			color: this.getRGB(element.fog,"color",true),
			near : this.getFloat(element.fog,"near",true),
			far : this.getFloat(element.fog,"far",true)
		};
		return newObj;
	}
	...
	getSkybox(element){
		...
		let newObj = {
			elem: "skybox",
			descriptor: "skybox",
			size: this.getVector3(element.skybox,"size",true),
			center : this.getVector3(element.skybox,"center",true),
			emissive : this.getRGB(element.skybox,"emissive",true),
			intensity : this.getFloat(element.skybox,"intensity",true),
			front : this.getString(element.skybox,"front",true),
			back : this.getString(element.skybox,"back",true),
			up : this.getString(element.skybox,"up",true),
			down : this.getString(element.skybox,"down",true),
			left : this.getString(element.skybox,"left",true),
			right : this.getString(element.skybox,"right",true),
	...
	*/
	loadJsonItem(options) {
		...
		for (let i = 0; i < options.descriptor.length; i++) {
			...
			else if (descriptor.type === "fog_def") {
				value = this.getFog(options.elem)
			}
			else if (descriptor.type === "skybox_def") {
				value = this.getSkybox(options.elem)
			}
	...
	loadNode(id, nodeElement) {
		...
		//SHADOWS
		obj.castshadows = nodeElement["castshadows"];
		obj.receiveshadows = nodeElement["receiveshadows"];
		...
		this.loadChildren(obj, children,lodNodes);
	...
	loadChildren(nodeObj, childrenElement,lodNodes) {
		for (let elem of lodNodes){
			
			let reference = this.data.getNode(elem.nodeId);

			if (reference === null) {
				// does not exist, yet. create it!
				reference = this.data.createEmptyNode(elem.nodeId);

				//Mindist need to be defined
				reference["mindist"] = elem.mindist
			}
			// reference it.
		...
		for (let child in childrenElement) {
			...
			if (child === "nodesList") {
				for (let elem of childrenElement[child]){
					let reference = this.data.getNode(elem);
					if (reference === null) {
						// does not exist, yet. create it!
						reference = this.data.createEmptyNode(elem);
					}
					// reference it.
					this.data.addChildToNode(nodeObj, reference)
				...
				for (let elem of childrenElement[child]){
					...
					if (reference === null) {
						// does not exist, yet. create it!
						reference = this.data.createEmptyLod(elem);
					}
					this.data.addChildToNode(nodeObj, reference)
				}
